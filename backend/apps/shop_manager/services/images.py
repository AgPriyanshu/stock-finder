...
def build_original_key(shop_id, content_type: str) -> str:
    ext = CONTENT_TYPE_TO_EXT.get(content_type)

    if not ext:
        raise ValueError(f"Unsupported content_type: {content_type}")

    return f"stock-finder/shops/{shop_id}/originals/{uuid.uuid4()}.{ext}"


def presign_put(shop_id, content_type: str) -> dict:
    key = build_original_key(shop_id, content_type)
